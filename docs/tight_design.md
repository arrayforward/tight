...
- [7. 拥塞控制设计（BBR 风格）](#7-拥塞控制设计bbr-风格)
...
sequenceDiagram
    ...
    L->>N: Online
    N->>L: Online（幂等重发直至收到）
    Note over L,N: Online 后各端发 100KB Probe 列车测带宽<br/>接收端实测带宽随 Report 回传给发送端播种 BBR
...
- 熵公式：`H(p) = -p·log₂p - (1-p)·log₂(1-p)`，冗余 = `max(H×1.2, p)`
  （高 p 区间冗余跟随 p 单调上升），100 片安全阀门；
- 通道固定冗余 `channel_fec_extra[8]`（如音频通道 +1~2 片）与 FEC 2× 探测
  冗余叠加其上，总校验片封顶 data_count。

### 6.3 FEC 2× 探测（无 L4S 路径）

带宽跟跌后（无 L4S 反馈），发送侧在 6s 时限内追加 2 片真实校验片，把线速
顶到 ~2×btl，用投递率样本重新发现链路余量；探测流量仅作用于通道 0
（视频），避免干扰音频稳定传输。
...
```mermaid
flowchart LR
    subgraph Sample["样本来源"]
        R1["投递率（接收端实测 recv_rate，丢包校正）"]
        R2["ACK 样本（bytes/rtt，只升不降）"]
        R3["迟到率（对端上报，次级信号）"]
        R4["CE 占比（L4S/ECN 反馈）"]
        R5["测速列车带宽（建连播种）"]
    end
    subgraph Est["BandwidthEstimator"]
        B1["BtlBw = 窗口 5 最大值滤波"]
        B2["RTprop = 最小 RTT"]
        B3["增益时间片 8×1.25 + 8×0.75（1:1）"]
    end
    subgraph Out["输出"]
        P1["令牌桶 pacing 速率"]
        P2["FEC 2× 探测标志"]
    ...
    Sample --> Est --> Out
    Out -->|"rate = btl × gain"| TB["令牌桶（cap = max(4×mtu, bps×0.02)）"]
...
| 规则 | 理由 |
|---|---|
| 投递率样本来自接收端实测 | 真实链路瓶颈，不受 ACK 游标跳缺影响 |
| 投递率 < 阈值（探测片 0.75×btl / 排空片 0.5×btl）即跟跌 | 无 L4S 兜底下降 + L4S 超发回落 |
| **app_limited 时绝不跟跌** | 投递率 = 应用速率而非链路容量，跟跌会形成螺旋坍缩 |
| **pacer_limited 样本拒绝采纳** | 排空片样本 ~0.6×BtlBw 会把估计拉垮 |
| ACK 样本只升不降 | ACK 只出现在握手期，RTT 可能被拥塞队列放大成几百 B/s 毒样本 |
| 爬升限速 ≤1.25×/样本 | 2× 曾致排空恢复后 btl 快速顶回旧高值，反复排空循环 |
| app_limited 判定 = 出站队列空 且 最近 500ms 无应用发送 | 持续流帧间空隙短暂为空不算 app_limited，否则 btl 卡死 |

### 7.3 增益时间片

- 增益只由时间片决定：8 个探测片（1.25）+ 8 个排空片（0.75），链路受限时
  探测积攒的队列恰好被排空片清空，平均速率 = BtlBw；
- RTT 不参与增益：应用速率恒大于链路时，纯 RTT 触发排空会**永久停在 0.75**
  （队列排不空 → RTT 不降 → 坍缩）；
- 排空片仅在探测片真正建过队列（`m_probe_pacer_limited`）时生效，否则跳过
  （btl ≈ 应用速率时排空只会自造永不排空的积压）。

### 7.4 L4S/ECN

- 出站报文标记 ECT(1)（RFC 9331），接收端统计 CE 标记占比上报；
- CE > 0：`btl = 最近投递率 R × (1 - 0.5×ce)`（相对实测收敛，有界）；CE = 0：
  `×2` 恢复爬升（封顶 2×最近投递率）；从未见 CE → 无 L4S 路径（FEC 探测）。

### 7.5 令牌桶与直发通道

- 数据面出站报文走 `m_outbound_queue` + 令牌桶 pacing（sender 线程消费）；
- **控制面直发**（绕过队列与令牌桶）：Report / 命令 / 握手重传 / NACK 重传。
  理由：报告在 echo 洪泛时排队 ~7s 会令投递率样本过期、btl 跟跌被拖延；
  重传量低（弱网 ~4%），直发不破坏限速语义。
...
| 6 | 增益由时间片决定、RTT 不参与 | RTT 趋势触发排空 | 应用速率恒 > 链路时 RTT 排空永久 0.75 坍缩 |
| 7 | 投递率样本来自接收端实测 | 发送端按 ack 估算 | 不受游标跳缺影响，是 BtlBw 的可靠测量源 |
| 8 | ACK 样本只升不降 | 直接写窗口 | 握手期 ACK 的 RTT 可能被放大 → 毒化 BtlBw |
| 9 | app_limited 用 500ms 无发送活动判定 | 出站队列空即判定 | 持续流的帧间空隙短暂为空，误判会卡死 btl |
| 10 | 报告/命令/重传直发绕过令牌桶 | 全部排队 | 报告排队 ~7s 使样本过期、btl 跟跌拖延；重传量低 |
| 11 | FEC 冗余按迟到率熵公式自适应 | 固定冗余率 | 弱网自动加强、干净链路零开销，迟滞防振荡 |
| 12 | 时钟对表 + 心跳平滑 | 只握手对表 | 跟踪晶振漂移；单程延迟统计依赖偏移 |
