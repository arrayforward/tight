...
flowchart LR
    ...
    subgraph Est["BandwidthEstimator（AIMD，量化阶梯 + 迟滞）"]
        B1["持续超发时按强度量化降速：<br/>≥50%→×0.20 · ≥20%→×0.30 · ≥5%→×0.45 · ≥1%→×0.65"]
        B2["恢复（延迟<10ms 且 迟到率<0.5%）两步台阶：×1.5 + FEC 探测冗余<br/>（CE 活跃跳过）→ 再 ×1.5 移除 FEC，连续爬升"]
        B3["中间区保持不动（防摆动）"]
        B4["下限 100kbps；提升上限 = 配置种子（默认 30Mbps）"]
    end
    subgraph Out["输出"]
        P1["令牌桶 pacing 速率"]
...
| **量化降速阶梯**（持续超发时，strength = max(late, ce)，令牌受限时 max(loss, ce)）：≥50% → ×0.20、≥20% → ×0.30、≥5% → ×0.45、≥1% → ×0.65、仅 delay 信号 → ×0.65 | 相比固定 ×0.5 每报告连降：超发积压排空期 CE 持续 → 连降 7 轮把 btl 崩到远低于真实链路（30M→0.23M）→ 视频下限都"超发"→ 贷款循环；量化大降一次到位 → btl 收敛在真实链路附近 → CE 即停 → 不崩底不循环 |
| 拥塞信号（带迟滞）：排队延迟 EWMA > 20ms（非令牌受限）或 迟到率 > 2%（非令牌受限）或 丢包率 > 2%（令牌受限时替代迟到率）或 CE > 1% | 实时流语义：延迟超线即"丢"；令牌受限时本地排队是伪信号，丢包/CE 才是链路信号（防"令牌卡死 → late 100% → 连降崩底"死锁） |
| **恢复判定更严（迟滞）**：延迟 < 10ms **且**（迟到率 < 0.5%，或令牌受限时丢包率 < 0.5% 且 CE < 1%）才提升；中间区保持不动 | 消除"刚过阈值就反向"的摆动（弱网段实测 btl 在 0.2~0.8M 振荡的根源） |
| 恢复两步台阶：第一步 ×1.5 + FEC 探测冗余，第二步无拥塞再 ×1.5 并移除 FEC，台阶走完回到起点连续爬升 | 台阶间隔一个报告周期观察，防抖动误提；FEC 可丢失、不伤业务；CE 活跃（L4S）时跳过 FEC 探测（CE 即链路反馈，探测冗余自伤） |
...
| **拥塞排空窗口（`slowdown_window_ms`，默认 3s）**：剧烈降速（×0.45 及以下档）后，窗口内 `video_capacity_bps` 输出排空码率 = btl 快照 − 超发积压 Q×8/窗口（Q = Σ(send−recv)×Δt 积分） | 应用编码码率骤降 → 发送骤减 → 积压排空 → CE 早停（排空期 btl 连降轮数少、不崩底）；窗口结束自动恢复，应用零改动（回调与轮询共用） |
| btl 下限 100kbps（`kMinBtlBps=12500`） | 长距离高 RTT 误判时防打穿 |
| 提升上限 = `initial_bandwidth_bytes` 种子（默认 30Mbps） | 防台阶把种子推高振荡；弱网下由报告量化收敛 |
...
- **排空窗口**（`slowdown_window_ms` > 0 且距 `last_congest_at()` 在窗口内）：
  输出排空码率 =（btl 快照 − 超发积压 Q×8/窗口 − 音频 − file/data）/（1+冗余），
  Q = Σ max(0, 发送速率−对端接收速率)×Δt（报告期积分），窗口结束自动恢复
  （btl 回升 → 码率回归跟随）；Q 不可得时减半兜底；
...
| 26 | 拥塞排空窗口（slowdown_window_ms=3s）：窗口内 video_capacity 输出排空码率 | 只降 btl 等自然排空 | 编码码率骤降 → 发送骤减 → 积压排空 → CE 早停（排空期 btl 连降轮数少、不崩底）；应用零改动自动恢复 |
