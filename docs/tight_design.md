...
| **拥塞排空窗口（slow Q 面积法，`slowdown_window_ms`）**：降幅 ≥20%（因子 ≤0.80：×0.75/×0.65/×0.50）时进入——快照超发积压 Q 与 btl，窗口内 cap =（btl_snap − Q×8/窗口 − 音频 − fd）/（1+冗余），3s 排完结束（Q 清零），**不跳帧**（统一柔表最大降 50%，播放端无需跳帧）；窗口内 btl 完全冻结；fast/IDR 排空路径保留在代码中（事件驱动清队列 + `EvacKeyframeCallback`），当前统一柔表不触发（因子下限 0.50 > fast 阈值 0.40） | 应用编码码率骤降 → 发送骤减 → 积压排空 → CE 早停；柔降不跳帧避免播放端跳变（编码器重启频率与无 CE 网络相当）；窗口结束自动恢复，应用零改动（回调与轮询共用） |
...
- **排空窗口（slow Q 面积法，`slowdown_window_ms` > 0 时）**：降幅 ≥20%
  （因子 ≤0.80）进入——快照超发积压 Q 与 btl，窗口内 cap =（btl 快照 −
  Q×8/窗口 − 音频 − fd）/（1+冗余），3s 排完结束（Q 清零）；**不跳帧**
  （统一柔表最大降 50%，播放端无需跳帧）；fast/IDR 排空路径保留在代码中
  （事件驱动清队列 + `EvacKeyframeCallback`），当前统一柔表不触发；
