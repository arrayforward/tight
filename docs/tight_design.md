...
flowchart LR
    subgraph Sample["信号来源（全部来自对端报告，每 report_interval 评估一次）"]
        ...
        R2["late-based：迟到率 p（超迟到线<br/>或 CE 标记报文占比，替代丢包率）"]
        R3["ECN/L4S：CE 标记在接收端计入迟到统计，自动并入 late-based"]
        R4["pacer_limited：本地令牌限速中<br/>（否决拥塞判定，防崩底死锁）"]
    end
    subgraph Est["BandwidthEstimator（AIMD，带迟滞）"]
        B1["拥塞（EWMA>20ms 或 p>2%）→ btl ×= 0.5"]
        B2["恢复（延迟<10ms 且 p<0.5%）两步台阶：×1.5 + FEC 探测冗余<br/>→ 无拥塞再 ×1.5 移除 FEC，连续爬升至种子"]
...
| 规则 | 理由 |
|---|---|
| 拥塞 = 排队延迟 EWMA > 20ms 或 迟到率 > 2% → btl ×= 0.5（每报告，无冷却） | 实时流语义：延迟超线即"丢"；以报告为准立即响应 |
| **恢复判定更严（迟滞）**：延迟 < 10ms **且** 迟到率 < 0.5% 才提升；中间区（10~20ms 或 0.5%~2%）保持不动 | 消除"刚过阈值就反向"的摆动（弱网段实测 btl 在 0.2~0.8M 振荡的根源） |
| **pacer_limited 否决拥塞判定** | 本地令牌不足的 p50 高是限速自造（btl 低估 → 令牌<供给 → 自造积压），走恢复台阶解除限速——否则"崩底 → 更崩"死锁 |
| 恢复两步台阶：第一步 ×1.5 + FEC 探测冗余，第二步无拥塞再 ×1.5 并移除 FEC，台阶走完回到起点连续爬升 | 台阶间隔一个报告周期观察，防抖动误提；FEC 可丢失、不伤业务；连续爬升直到种子上限或拥塞信号（step=2 保持曾致 btl 卡在中途永远到不了真实带宽） |
...
### 7.4 FEC 关闭（RTT >200ms）

平滑 RTT 长期 >200ms（长距离或重拥塞）→ `peer.m_fec_disable` 置位：
起始保护/自适应/通道额外/探测冗余**全部归零**——少量阻塞时冗余恢复有用，
大量阻塞时冗余本身挤占带宽加剧拥塞，让出带宽给数据；回落自动恢复。
