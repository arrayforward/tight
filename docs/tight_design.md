...
flowchart LR
    ...
    subgraph Est["BandwidthEstimator（AIMD，双系数表 + 迟滞 + 排空冻结）"]
        B1["持续超发时按信号来源分表降速：<br/>late/delay 主导（柔表）≥50%→×0.50 … ≥1%→×0.90<br/>CE 主导（急表）≥50%→×0.20 … ≥1%→×0.65"]
...
| **双系数降速表**（持续超发时按信号来源分流）：<br/>**柔表（late/delay 主导，软信号**——迟到/排队可能含追赶、本地令牌拖帧成分）：≥50%→×0.50、≥20%→×0.65、≥5%→×0.75、≥1%→×0.90，最大降 50%、slow 排空不跳帧<br/>**急表（CE 主导，硬信号**——proxy 直测队列积压 = 真实超发）：≥50%→×0.20、≥20%→×0.30、≥5%→×0.45、≥1%→×0.65，≥20% 即 fast 跳帧 | 统一急表（×0.2）崩底：实测 4M/30M 链路 btl 被 late 追赶误判一路打到 656K（视频下限以下）→ 贷款循环——柔表让误判温和收敛、真超发（CE）仍急降 |
...
| **拥塞排空窗口（双模式）**：<br/>**fast**（降幅 >50%，因子 <0.40：CE ×0.20/×0.30）——清空视频积压（`clear_outbound` 保留音频）+ 通道排空 100ms + 回调应用重启编码器出新 IDR（播放端跳帧、积压瞬间归零）；窗口事件驱动结束 = 新 IDR 提交发送 + 下一报告到达，`kEvacTimeout` 5s 兜底<br/>**slow**（降幅 20%~50%，因子 0.40~0.80：柔表全部 + CE ×0.45/×0.65）——Q 面积法：快照超发积压 Q 与 btl，窗口内 cap = (btl_snap − Q×8/窗口)/(1+冗余)，3s 排完结束（Q 清零） | 应用编码码率骤降 → 发送骤减 → 积压排空 → CE 早停；fast 清队列跳帧是硬止损（播放端跳到新 IDR 时间线），slow 温和排空不跳帧（软信号误判不伤业务）；窗口结束自动恢复，应用零改动（回调与轮询共用） |
...
- **排空窗口（双模式，`slowdown_window_ms` > 0 时）**：
  - **fast**（降幅 >50%，因子 <0.40：CE ×0.20/×0.30）：清空视频积压（`clear_outbound` 保留音频）+ 视频通道排空 100ms + `EvacKeyframeCallback` 通知应用重启编码器出新 IDR——播放端跳到新 IDR 时间线、追赶/超发积压瞬间归零；窗口事件驱动结束 = **新 IDR 提交发送（`send_video(keyframe=true)`）+ 下一报告到达**，`kEvacTimeout` 5s 兜底；关键帧传输期间冻结期延长（`kKeyframeHoldTotal` 800ms——IDR 60KB 低带宽传输 27-33ms 触达 L4S 阈值 → proxy CE，突刺 CE 是帧自身传输，冻结期内不误判真超发急降）；
  - **slow**（降幅 20%~50%，因子 0.40~0.80：柔表全部 + CE ×0.45/×0.65）：Q 面积法——快照超发积压 Q 与 btl，窗口内 cap =（btl 快照 − Q×8/窗口 − 音频 − fd）/（1+冗余），3s 排完结束（Q 清零）；
  - **窗口内 btl 完全冻结**（on_report 的降速与恢复台阶均豁免）；窗口结束自动恢复（btl 回升 → 码率回归跟随）；
...
| 29 | 双系数降速表（late/delay 柔表 ×0.50~0.90 vs CE 急表 ×0.20~0.65） | 统一急表 | 统一急表（×0.2 崩底）：late 追赶误判一路打到 656K（视频下限以下）→ 贷款循环；柔表让误判温和收敛、真超发（CE）仍急降 |
