...
flowchart LR
    ...
    subgraph Est["BandwidthEstimator（AIMD）"]
        B1["拥塞（EWMA>20ms 或 p>1%）→ btl ×= 0.5"]
        B2["恢复两步台阶：×1.5 + FEC 探测冗余<br/>→ 无拥塞再 ×1.5 移除 FEC"]
        B3["下限 100kbps；提升上限 = 配置种子"]
    end
    subgraph Out["输出"]
        P1["令牌桶 pacing 速率"]
...
| 规则 | 理由 |
|---|---|
| 拥塞 = 排队延迟 EWMA > 20ms 或 迟到率 > 1% → btl ×= 0.5（每报告，无冷却） | 实时流语义：延迟超线即"丢"；以报告为准立即响应 |
| **pacer_limited 否决拥塞判定** | 本地令牌不足的 p50 高是限速自造（btl 低估 → 令牌<供给 → 自造积压），走恢复台阶解除限速——否则"崩底 → 更崩"死锁 |
| 恢复两步台阶：第一步 ×1.5 + FEC 探测冗余，第二步无拥塞再 ×1.5 并移除 FEC | 台阶间隔一个报告周期观察，防抖动误提；FEC 可丢失、不伤业务 |
