...
- 迟到率 p 是带宽估计器的**主拥塞信号之一**（late-based）：p 超过 1% 阈值即判定拥塞、btl 减半（与 delay-based 排队延迟信号共同构成三信号 AIMD，详见 §6.6）；
- 恢复提升采用**两步台阶法**：第一步追加 2 片 FEC 探测校验分片压上负载感知链路（FEC 可丢失、不伤业务），第二步确认链路有余量后移除 FEC、业务流量自然替换；
...
RTprop 的 EWMA 平滑，超 20ms 阈值）+ late-based（迟到率 p 超 1% 阈值）
判定拥塞时 btl ×= 0.5；恢复采用两步台阶 ×1.5（提升上限为配置种子，
btl 下限 100kbps），第一步先追加 2 片 FEC 校验片压上负载感知链路（FEC
可丢失、不伤业务），第二步确认链路有余量后移除 FEC、业务流量自然替换
（视频可用码率按实际 FEC 冗余率折算）。本地令牌桶限速中（pacer_limited）
否决拥塞判定，防止"崩底 → 令牌不足 → 自造积压 → 误判拥塞"死锁；平滑
RTT 长期超过 200ms 时关闭 FEC 冗余，让出带宽给数据。
