...
接收速率（关键帧突刺）时不降速。持续超发时按**信号来源双表降速**：
late/delay 主导（软信号，柔表 ×0.90/×0.75/×0.65/×0.50，最大降 50%、
slow 排空不跳帧——统一急表曾把 btl 打到 656K 视频下限以下 → 贷款循环），
CE 主导（硬信号，急表 ×0.65/×0.45/×0.30/×0.20，≥20% 即 fast 跳帧）。
...
自然替换（视频可用码率按实际 FEC 冗余率折算）。剧烈降速后进入**拥塞排空
窗口双模式**：fast（CE 急降 >50%，清空视频积压 + 通道排空 + 通知应用重启
编码器出新 IDR，播放端跳帧、积压瞬间归零；窗口结束 = 新 IDR 提交发送 +
下一报告到达，5s 兜底）/ slow（柔降 20%~50%，Q 面积法 3s 排完积压）。
