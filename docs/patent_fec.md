...
判定拥塞时 btl ×= 0.5；恢复判定更严（延迟 <10ms 且迟到率 <0.5% 才提升，
中间区保持防摆动），采用两步台阶 ×1.5（提升上限为配置种子，btl 下限
100kbps，台阶走完连续爬升），第一步先追加 2 片 FEC 校验片压上负载感知
链路（FEC 可丢失、不伤业务），第二步确认链路有余量后移除 FEC、业务
流量自然替换（视频可用码率按实际 FEC 冗余率折算）。本地令牌桶限速中
（pacer_limited）否决拥塞判定，防止"崩底 → 令牌不足 → 自造积压 → 误判
拥塞"死锁；平滑 RTT 长期超过 200ms 时关闭 FEC 冗余，让出带宽给数据。
