...
```
冗余率 r = max( H(p) × 1.2 , p )
校验片数 = ceil( data_count × r )，下限 1，上限 20%（ceil(data×0.2)，再受 100 安全阀门）
其中 H(p) = -p·log2(p) - (1-p)·log2(1-p)
```

本发明对纯熵公式作出两处改进：**其一**，乘以安全系数 1.2 使冗余略高于理论下限（覆盖统计噪声）；**其二**，以 `max(H(p)×1.2, p)` 补偿高 p 区间——H(p) 在 p→1 时趋 0，直接采纳会导致链路严重劣化时冗余骤降，取 max 后冗余率在高 p 区间跟随 p 单调上升，保护强度与损伤程度始终正相关。**冗余率上限 20%**：拥塞/长尾场景冗余过大加剧排队（L4S 弱网实测 fec=100% 使线上超发、恶性循环）；单分片消息至少 1 片保护（100% 冗余是必要最小保护）。
...
- 迟到率 p 是带宽估计器的**主拥塞信号之一**（late-based）：p 超过 2% 阈值即判定拥塞、btl 减半（与 delay-based 排队延迟信号共同构成三信号 AIMD，详见 §6.6）；
- 恢复提升采用**两步台阶法**（迟滞：延迟 <10ms 且迟到率 <0.5% 才提升，中间区保持）：第一步追加 2 片 FEC 探测校验分片压上负载感知链路（FEC 可丢失、不伤业务），第二步确认链路有余量后移除 FEC、业务流量自然替换；
- 编码侧带宽预算联动：`video_capacity_bps` 按"有效带宽 − 音频预留 − file/data 实时速率"再除以实际 FEC 冗余率折算视频可用码率（本申请附图省略，属可选的从属实现）。
...
6. **拥塞联动**：迟到率与排队延迟共同驱动 AIMD 拥塞判定（btl 减半），恢复提升用 FEC 探测冗余先行感知链路余量，冗余与限速相互配合，链路利用率不因冗余浪费；
...
3. 叠加通道固定冗余（channel_fec_extra[channel]）与 AIMD 恢复台阶的 FEC 探测冗余（2 片，仅视频通道）；RTT>200ms 时冗余全部归零（让出带宽）；
...
带宽估计器采用三信号 AIMD（GCC 风格）：delay-based（排队延迟 = P50 −
RTprop 的 EWMA 平滑，超 20ms 阈值）+ late-based（**帧级迟到率**——帧延迟
> F/btl + late_buffer 才算迟到，关键帧突刺是帧自身传输不误报）+ loss-based
+ ECN（CE 占比超 1% 直接判定，L4S 下 CE 即丢包替代；CE 最小样本门槛 <20
报文不判，防小分母放大尾流虚高）。**突刺门控**：报告期平均发送 ≤ 对端
接收速率（关键帧突刺）时不降速。持续超发时按**统一柔表**降速（strength
= max(late, loss, ce)，CE 与 late/loss 共享：≥50%→×0.50、≥20%→×0.65、
≥5%→×0.75、≥1%→×0.90，最大降 50%、不跳帧——CE 急表曾致 btl/码率大幅
跳变 → QSV 编码器频繁重启（60s 17-36 次）→ 设备崩溃/断流，柔降渐变收敛
后重启次数与无 CE 网络相当（~12-15 次）、设备稳定）。
**排空窗口内 btl 完全冻结**（不降不升，排空/追赶期迟到信号是伪拥塞），
窗口结束（3s）后信号仍在才允许下一次下降。恢复判定更严（延迟 <10ms 且
迟到率 <0.5% 才提升，中间区保持防摆动），采用两步台阶 ×1.5（提升上限 =
min(配置种子, 对端接收速率×1.2)——快排跳帧后爬升失去反馈会立即爬满格
又超发，recv_rate 是链路真实吞吐约束收敛；btl 下限 100kbps，台阶走完
连续爬升），第一步先追加 2 片 FEC 校验片压上负载感知链路（FEC 可丢失、
不伤业务；**CE 活跃时跳过 FEC 探测**——CE 即链路反馈，探测冗余使线上
超发 → 更多 CE → 连降自伤），第二步确认链路有余量后移除 FEC、业务流量
自然替换（视频可用码率按实际 FEC 冗余率折算）。降幅 ≥35%（strength≥20%）
后进入**拥塞排空窗口 fast 模式**（清空视频积压 + 通道排空 + 通知应用重启
编码器出新 IDR，播放端跳帧、积压瞬间归零；窗口结束 = 新 IDR 提交发送 +
下一报告到达，5s 兜底）；降幅 25%（×0.75）走 slow 模式（Q 面积法：快照
超发积压 Q 与 btl，窗口内输出排空码率，3s 排完不跳帧）；窗口内 btl 完全
冻结。
**起步带宽校准**：握手后首个测速报告将 btl 钳制 ≤ 实测链路容量（不锁
种子——固定 30M 种子在 4M 链路硬发 → 大步量化崩底；probe 测偏只慢爬不
卡死）。本地令牌桶限速中（pacer_limited）只信 CE（late/loss 是本地限速
伪信号），防止"崩底 → 令牌不足 → 自造积压 → 误判拥塞"死锁；平滑 RTT
长期超过 200ms 或 CE>1%（L4S）时关闭 FEC 冗余让出带宽给数据（丢包型随机
丢包不关——丢包正是 FEC 的工作对象）。**报告停滞降速**：对端 Report 连续
3×report_interval 未到达（报告直发绕过令牌桶与队列，本地限速不会丢报告）
= 链路严重卡顿/断流 → btl ×0.5 单次降（one-shot，下限 100kbps），报告
恢复后由恢复台阶自然回升——该信号早于贷款硬止损（~5s）与掉线检测
（30s）；仅 Online/Established 链路且全部活跃 peer 均停滞才触发，握手时
重置报告接收时间戳防重连误触发。
...
| 拥塞联动 | 相互独立 | 迟到率 + 排队延迟驱动 AIMD 判定（btl 减半），恢复用 **FEC 探测冗余**先行感知链路余量 |
...
| FEC **探测冗余**与迟到率/排队延迟拥塞联动（AIMD 恢复台阶） | 基于丢包率/RTT 的 AIMD 类拥塞控制公知；"恢复提升第一步追加 FEC 校验片压上负载感知链路余量（可丢失、不伤业务）、第二步业务替换"未检索到公开 | 具潜在新颖性 |
