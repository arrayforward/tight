...
RTprop 的 EWMA 平滑，超 20ms 阈值）+ late-based（迟到率 p 超 2% 阈值）
+ loss-based（令牌受限时以丢包率替代迟到率——本地排队是伪信号）+ ECN
（CE 占比超 1% 直接判定，L4S 下 CE 即丢包替代、丢包不再额外计入迟到率）
判定拥塞时 btl 乘性下降（正常 ×0.5，令牌受限 ×0.75 温和降防本地排队）；
恢复判定更严（延迟 <10ms 且迟到率 <0.5% 才提升，中间区保持防摆动），
采用两步台阶 ×1.5（提升上限为配置种子，btl 下限 100kbps，台阶走完连续
爬升），第一步先追加 2 片 FEC 校验片压上负载感知链路（FEC 可丢失、不伤
业务；**CE 活跃时跳过 FEC 探测**——CE 即链路反馈，探测冗余使线上超发 →
更多 CE → 连降自伤），第二步确认链路有余量后移除 FEC、业务流量自然替换
（视频可用码率按实际 FEC 冗余率折算）。本地令牌桶限速中（pacer_limited）
迟到/延迟信号豁免，防止"崩底 → 令牌不足 → 自造积压 → 误判拥塞"死锁；
平滑 RTT 长期超过 200ms 或 CE>1%（L4S）时关闭 FEC 冗余让出带宽给数据
（丢包型随机丢包不关——丢包正是 FEC 的工作对象）。
