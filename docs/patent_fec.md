...
（令牌受限时以丢包率替代迟到率——本地排队是伪信号）+ ECN（CE 占比超 1%
直接判定，L4S 下 CE 即丢包替代、丢包不再额外计入迟到率）。**突刺门控**：
报告期平均发送 ≤ 对端接收速率（关键帧突刺）时不降速，btl 保持、突刺排空
后恢复台阶自然回升——避免关键帧排队把 btl 打崩（30M→0.23M 崩底 → 贷款
循环）。持续超发时按信号强度**量化降速**（≥50%→×0.20、≥20%→×0.30、
≥5%→×0.45、≥1%→×0.65，一次报告收敛到位，避免固定 ×0.5 连降把 btl 崩到
远低于真实链路）。恢复判定更严（延迟 <10ms 且迟到率 <0.5% 才提升，中间区
保持防摆动），采用两步台阶 ×1.5（提升上限为配置种子，btl 下限 100kbps，
台阶走完连续爬升），第一步先追加 2 片 FEC 校验片压上负载感知链路（FEC
可丢失、不伤业务；**CE 活跃时跳过 FEC 探测**——CE 即链路反馈，探测冗余
使线上超发 → 更多 CE → 连降自伤），第二步确认链路有余量后移除 FEC、业务
流量自然替换（视频可用码率按实际 FEC 冗余率折算）。剧烈降速后进入
**拥塞排空窗口**（`slowdown_window_ms`，默认 3s）：窗口内视频码率输出排空
值（btl 快照 − 超发积压 Q×8/窗口），3s 内排完积压 → 发送骤减 → CE 早停 →
不崩底，窗口结束自动恢复。本地令牌桶限速中（pacer_limited）迟到/延迟
信号豁免，防止"崩底 → 令牌不足 → 自造积压 → 误判拥塞"死锁；平滑 RTT
