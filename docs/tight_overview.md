...
flowchart LR
    ...
    subgraph E["BandwidthEstimator（AIMD，双系数表 + 迟滞 + 排空冻结）"]
...
- **双系数降速表**（持续超发时按信号来源分流）：**柔表（late/delay 主导，
  软信号**——可能含追赶/本地令牌拖帧成分）：≥50%→×0.50、≥20%→×0.65、
  ≥5%→×0.75、≥1%→×0.90，最大降 50%、slow 排空不跳帧；**急表（CE 主导，
  硬信号**——proxy 直测队列积压）：≥50%→×0.20、≥20%→×0.30、≥5%→×0.45、
  ≥1%→×0.65，≥20% 即 fast 跳帧。统一急表（×0.2）崩底：late 追赶误判一路
  打到 656K（视频下限以下）→ 贷款循环；
...
- **拥塞排空窗口双模式**：fast（CE 急降 >50%，清空视频积压 + 通道排空 +
  `EvacKeyframeCallback` 重启编码器出新 IDR，播放端跳帧、积压瞬间归零；
  窗口结束 = 新 IDR 提交发送（`send_video(keyframe=true)`）+ 下一报告到达，
  5s 兜底）/ slow（柔降 20%~50%，Q 面积法 3s 排完积压，不跳帧）；
