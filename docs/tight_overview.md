...
- **拥塞排空窗口（slow Q 面积法）**：降幅 ≥20%（因子 ≤0.80）进入——快照
  超发积压 Q 与 btl，窗口内 `video_capacity_bps` 输出排空码率（btl 快照 −
  Q×8/窗口），3s 排完结束（Q 清零），**不跳帧**（统一柔表最大降 50%）；
  fast/IDR 清队列路径保留在代码中（当前柔表因子下限 0.50 不触发）；
