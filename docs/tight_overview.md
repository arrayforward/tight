...
flowchart LR
    subgraph S["信号（全部来自对端报告，每 report_interval 评估一次）"]
        ...
        R2["late-based：迟到率 p（超迟到线或丢失）"]
        R3["loss-based：纯丢包率（令牌受限时替代迟到率——本地排队是伪信号）"]
        R4["ECN/L4S：CE 标记占比（直接拥塞信号，>1% 即判）"]
        R5["pacer_limited：本地令牌限速中（迟到/延迟信号豁免，防崩底死锁）"]
    end
    subgraph E["BandwidthEstimator（AIMD，带迟滞）"]
        B1["拥塞（EWMA>20ms 或 p>2% 或 丢包率>2% 或 CE>1%）→ btl 乘性下降<br/>（正常 ×0.5；令牌受限 ×0.75 温和降）"]
...
- 拥塞判定（带迟滞）：排队延迟 EWMA > 20ms、迟到率 > 2%、**丢包率 > 2%**
  （令牌受限时替代迟到率——本地排队是伪信号，丢包才是链路真信号）或
  **CE 占比 > 1%**（L4S 直接信号）任一命中 → btl 乘性下降（正常 ×0.5，
  令牌受限 ×0.75 温和降防本地排队）；恢复判定更严：排队延迟 < 10ms **且**
  迟到率 < 0.5% 才提升；中间区（10~20ms 或 0.5%~2%）保持不动——消除
  "刚过阈值就反向"的摆动（弱网段实测 btl 在 0.2~0.8M 振荡的根源）；
- 恢复：两步台阶法，每步 ×1.5（共 +125%），台阶间隔一个报告周期观察；
  第一步先压 FEC 校验片负载感知链路（FEC 可丢失、不伤业务），第二步确认
  有余量后移除 FEC、业务流量自然替换（video_capacity 按实际冗余率折算）；
  **CE 活跃（L4S）时跳过 FEC 探测**（CE 即链路反馈，探测冗余使线上超发 →
  更多 CE → 连降自伤）；台阶走完后回到起点继续爬升，直到种子上限或拥塞信号；
