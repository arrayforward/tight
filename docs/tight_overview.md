...
flowchart LR
    ...
    subgraph E["BandwidthEstimator（AIMD，量化阶梯 + 迟滞）"]
        B1["持续超发时按强度量化降速：<br/>≥50%→×0.20 · ≥20%→×0.30 · ≥5%→×0.45 · ≥1%→×0.65"]
        B2["恢复（延迟<10ms 且 p<0.5%）两步台阶 ×1.5：<br/>第一步 + FEC 探测冗余（CE 活跃时跳过探测）<br/>第二步移除 FEC（业务替换），连续爬升至种子上限"]
        B3["中间区（10~20ms 或 0.5%~2%）保持不动（防摆动）"]
        B4["btl 下限 100kbps；提升上限 = 配置种子（默认 30Mbps）"]
    end
    subgraph O["输出"]
        P1["令牌桶 pacing 速率"]
...
- **量化降速阶梯**（持续超发时，strength = max(late, ce)，令牌受限时
  max(loss, ce)）：≥50% → ×0.20、≥20% → ×0.30、≥5% → ×0.45、≥1% → ×0.65、
  仅 delay 信号 → ×0.65——一次报告收敛到位，避免固定 ×0.5 连降把 btl 崩到
  远低于真实链路（30M→0.23M）→ 视频下限都"超发"→ 贷款循环；
- **帧级迟到率**：接收端逐帧统计（帧延迟 = 完成时刻 − 首片发送 tick、帧
  大小 F），发送端以合理到达时间 = F/btl + late_buffer 判定迟到——关键帧
  突刺是帧自身传输不误报，持续超发排队超过最大帧合理时间才记迟到；
- **拥塞排空窗口**（`slowdown_window_ms`，默认 3s）：剧烈降速（×0.45 及
  以下档）后窗口内 `video_capacity_bps` 输出排空码率（btl 快照 − 超发积压
  Q×8/窗口）——编码码率骤降 → 发送骤减 → 积压排空 → CE 早停（排空期 btl
  连降轮数少、不崩底）；窗口结束自动恢复，应用零改动；
- 恢复判定更严（迟滞）：延迟 < 10ms **且** 迟到率 < 0.5% 才提升；中间区
  （10~20ms 或 0.5%~2%）保持不动——消除"刚过阈值就反向"的摆动；
- 恢复两步台阶 ×1.5（FEC 探测先行；CE 活跃跳过），台阶走完回到起点连续
  爬升，直到种子上限（默认 30Mbps）或拥塞信号；
- **令牌受限豁免**：令牌卡死时迟到率主体是本地排队（应用码率下限 > 令牌 →
  p50 超线），照用会"连降崩底 12.5K"死锁——只用丢包率与 CE 判定；
...
- **报告停滞降速**：Report 连续 3×report_interval 收不到 = 链路严重卡顿/断流
  （报告直发绕过令牌桶，本地限速不会丢报告）→ btl ×0.5 **单次降**（one-shot
  防盲猜过冲，下限 100kbps），报告恢复后恢复台阶自然回升——提前于贷款硬
  止损（~5s）与掉线检测（30s）；仅 Online/Established 链路、全部活跃 peer
  均停滞才降（网关多设备不被单设备停摆拖累），握手时重置报告时间戳防
  重连误触发；
