...
- **三信号 AIMD 拥塞控制（GCC 风格）**：delay-based（排队延迟 = P50−RTprop，
  ...
  + CE（>1% 直接信号）；**突刺门控**（非持续超发不降速）；持续超发按
  **信号来源双表降速**（late/delay 柔表 ×0.50~0.90 温和、CE 急表 ×0.20~0.65
  快速收敛）；**排空窗口内 btl 冻结**；恢复需 <10ms 且 <0.5%，两步台阶 ×1.5
  （上限 = min(种子, recv×1.2)）；**起步 probe 校准**（btl 不超实测链路）；
  btl 下限 100kbps，提升上限 = 配置种子（默认 30Mbps）
...
- **拥塞排空窗口双模式**：fast（CE 急降 >50% → 清空视频积压 + `EvacKeyframeCallback`
  通知应用重启编码器出新 IDR，播放端跳帧积压归零）/ slow（柔降 20%~50% →
  Q 面积法 3s 温和排空不跳帧）；窗口内 btl 冻结
- **`send_video(peer, payload, keyframe)`**：视频帧发送（通道 0 + 贷款连发），
  keyframe 标记 IDR 供 fast 窗口识别"新 IDR 已提交"作为结束条件
